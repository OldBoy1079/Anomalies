...
def prepare_data_for_prefixspan(data):
    """Преобразует numpy arrays в список списков строк"""
    prepared_data = []
...
def get_graph_actual_soh(data, frequent_itemset, names, ic, id, vc, vd, tc, td, color, n):
    ...
    for i in range(len(x1)):
        ...
        if (found and not limit):
            print(f'Вас заметили! Итерация: {i}')
...
eventsl=get_graph_actual_soh(data[100],frequent_itemsets_low,names1,ic,id,vc,vd,tc,td,"green","low")
# eventsm=get_graph_actual_soh_fixed(data[-1],frequent_itemsets_midum,names2,ic,id,vc,vd,tc,td,"orange","medium")
# eventsh=get_graph_actual_soh_fixed(data[-1],frequent_itemsets_high,names3,ic,id,vc,vd,tc,td,"red","high")
